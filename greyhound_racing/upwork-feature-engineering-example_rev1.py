...
# Percentage of top 3 finishes in last three races
data['Placings3'] = (data.groupby('NAME')['PLACED'].apply(lambda x: x.rolling(3).sum().shift()))
data['STARTS3'] = (data.groupby('NAME')['FINISH'].apply(lambda x: x.rolling(3).count().shift()))
data['PlaceP3'] = (data['Placings3'] / data['STARTS3']).round(2)

# Percentage of top 3 finishes in last twelve races
data['Placings12'] = (data.groupby('NAME')['PLACED'].apply(lambda x: x.rolling(12).sum().shift()))
data['STARTS12'] = (data.groupby('NAME')['FINISH'].apply(lambda x: x.rolling(12).count().shift()))
data['PlaceP12'] = (data['Placings12'] / data['STARTS12']).round(2)


# In[14]:

# Percentage of top 1 finishes in last three races
data['Wins3'] = (data.groupby('NAME')['WIN'].apply(lambda x: x.rolling(3).sum().shift()))
data['WinP3'] = (data['Wins3'] / data['STARTS3']).round(2)

# Percentage of top 1 finishes in last twelve races
data['Wins12'] = (data.groupby('NAME')['WIN'].apply(lambda x: x.rolling(12).sum().shift()))
data['WinP12'] = (data['Wins12'] / data['STARTS12']).round(2)
